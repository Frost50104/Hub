...
async def _on_event(session: AsyncSession, event: DeletionEvent) -> None:
    ...
    #
    # Повторная обработка события безопасна, но с 01.09 по ДРУГОЙ причине:
    # `archive_profile` с причиной `auth_deleted` обнуляет `employee_id`, и на
    # втором проходе выборка ниже просто не найдёт профиль. Идемпотентность
    # самого `archive_profile` (уже архивный → no-op) при этом никуда не делась
    # — она просто перестала быть тем, что нас спасает.
    log.info(
        "deletion_sync.event",
        seq=event.seq,
