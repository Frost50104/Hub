...
- задача заводится тем же `services/tasks.py::create_task_record`, что и любая
  другая, но с `watch_creator=False`: побочки create молчат и членства не дают.
  Подписка автора и его viewer-членство в проекте обратной связи идут явными
  шагами в ручке (решение владельца 02.09: автор получает пуши по своей
  задаче и открывает её по ссылке; следствие «видит проект целиком» принято);
