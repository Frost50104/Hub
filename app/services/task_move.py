...
async def assert_movable(
    *,
    ...
    # Шаблон (0060) в переносе не участвует ни с какой стороны: перенос
    # область шаблона не открывает (обе стороны и так дали бы 404), а триггер
    # `tasks_template_guard` отклонил бы смену стороны. Это третья линия — с
    # понятным текстом для того, кто всё-таки дошёл.
    if source.is_template or target.is_template:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Задачи шаблона не переносятся — используйте копирование проекта",
        )
