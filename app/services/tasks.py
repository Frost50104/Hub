...
def reject_legacy_status(value: object) -> None:
    """Старый QUERY-параметр `status` в списках — честная 422, а не тихий no-op.

    Четырёх системных статусов больше нет (0044). В ТЕЛЕ запроса legacy ловит
    `extra="forbid"` на схемах (0045), а вот неизвестный query-параметр FastAPI
    игнорирует молча — и старый бандл получил бы НЕотфильтрованный список
    вместо отфильтрованного (в т.ч. чип «Просроченные» слал `status=open`).
    Отсюда явный параметр в трёх ручках списков: `tasks`, `me_tasks`,
    `calendar`. Снимать — когда до старых бандлов уже не докричаться,
    отдельной задачей (`docs/tech-debt/open.md`).
    """
