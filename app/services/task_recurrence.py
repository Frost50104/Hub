...
async def spawn_next(
    ...
    """
    # Шаблон (0060) копий по повтору не порождает — и правило-токен ОБЯЗАНО
    # остаться на задаче шаблона, поэтому выход строго ДО `claim_rule`.
    # Галочку в шаблоне и так не поставить (409 в ручке), это вторая линия.
    if task.is_template:
        return None
