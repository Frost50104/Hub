...
class RetrievedChunk:
    """Лексический аналог RagChunk (провайдеры без embeddings API)."""
...
async def retrieve_lexical(
    ...
    *,
    ...
    """Fallback-retrieval: FTS по search_documents (та же audience-фильтрация).

    Используется, когда провайдер не умеет эмбеддинги (DeepSeek): семантика
    слабее, но граундинг и фильтр аудитории сохраняются полностью.
    """
    # Вопросительная фраза → OR-семантика по значимым словам: websearch
    # с AND почти никогда не матчит целый вопрос («Какие правила …?»).
    words = re.findall(r"[а-яёa-z0-9]{3,}", question.lower())[:8]
    query = " OR ".join(words) if words else question.strip()[:200]
    fts = text("search_vector @@ websearch_to_tsquery('russian', :q)").bindparams(
        q=query
    )
    docs = (
        (
            await db.execute(
                ...
                .where(
                    visible_filter(SearchDocument, profile_id),
                    or_(fts, SearchDocument.title.ilike(f"%{question.strip()[:80]}%")),
                )
                .order_by(
                    text(
    ...
    return [
        RetrievedChunk(
            ...
            title=d.title,
            url_path=d.url_path,
            content=" ".join(filter(None, [d.snippet, d.body_text]))[:1500]
            or d.title,
        )
        for d in docs
    ]
...
def _sources(chunks: list[RagChunk] | list[RetrievedChunk]) -> list[SourceRef]:
    ...
    try:
        try:
            ...
            chunks = await retrieve_lexical(
                db, question=body.question, profile_id=profile.id
            )
        context = "\n\n".join(
