...
- **S3 offsite backup** — флаг `BACKUP_S3_BUCKET` поддерживается в `scripts/backup-pg.sh` и `backup-files.sh`, но S3-клиента на хосте нет и креды не настроены. При потере VPS-диска восстановление невозможно без offsite-копии. Решение владельца 31.08 — Timeweb S3, ждёт бакет и ключи. **Включать «как есть» НЕЛЬЗЯ:** `backup-files.sh` синкует весь `$BACKUP_ROOT`, а hardlink'и в S3 не переживают — 14 снапшотов по 1,5 ГБ уедут как ~21 ГБ вместо 1,5. Синкать только `latest/` и только прод-дампы (staging воспроизводим, и это лишняя копия тех же перс. данных). `awscli` в Ubuntu 24.04 отсутствует (`Candidate: none`) — брать `rclone` 1.60.1 или `s3cmd` 2.4.0 из apt, а не вендорный установщик на 250 МБ.
