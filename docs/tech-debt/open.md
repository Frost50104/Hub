...
## Архив: создание закрыто, остальное — нет (28.08)

**Закрыто.** Новые задачи в архивный проект не заводятся: гейт
`services/projects.py::assert_project_accepts_tasks` стоит в общем пути
...
- **`t_search_tasks` и `t_my_tasks` архивные проекты не фильтруют**, хотя
  `/api/search` фильтрует (`api/search.py:241`). Это про поведение ассистента
  на ЧТЕНИЕ, а не про исчезающую работу: `/me/tasks` их тоже отдаёт, и фильтр в
  общей `visible_projects_stmt` сделал бы ассистента строже интерфейса.
