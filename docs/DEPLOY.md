...
3. Frontend: **собирается ЛОКАЛЬНО** (`npm install && npm run build[:staging]` в `web/`, проверки: `dist/index.html` и `dist/sw.js` есть, `index.html` не в прекеше `sw.js`, в `index.html` нет регистрации плагином — `registerSW`/`serviceWorker.register`/`vite-plugin-pwa:`, в `sw.js` нет `workbox-window`), затем `rsync` **БЕЗ `--delete`** внутри `web/dist/` на VPS + чистка по возрасту — см. §«Фронт: старые чанки не удаляются (16.09)» выше. С 2026-08-21 `vite build` на сервере не запускается: сборка росла до ~970 МБ RSS и падала по OOM даже при остановленном STT — и расширение машины до 4 ГБ этого решения не отменяет (см. §«Память VPS» ниже). node на сервере для деплоя больше не нужен.
