...
8. **Регистрация воркера — `web/src/lib/serviceWorker.ts` (25.09), не плагин vite-plugin-pwa.** Push от способа регистрации не зависит: `usePush`, `usePushAutoRefresh` и `revokePushBinding` ждут `navigator.serviceWorker.ready` / `getRegistration()` — свойства регистрации. Тихая активация нового воркера подписку не трогает (она принадлежит регистрации, не воркеру). Проверка после правок регистрации — тестовый пуш из «Настройки → Уведомления».
