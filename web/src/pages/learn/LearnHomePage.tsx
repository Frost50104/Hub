    ...
    <div className="mx-auto max-w-[680px] lg:max-w-[1000px] lg:px-4">
      ...
      {(archived || needsRestore) && (
        <div className="mx-5 mt-5 flex items-start gap-3 rounded-[14px] border border-hair bg-tint p-4">
          ...
          <div>
            {/* «Ожидает восстановления» тут стояло, пока `needs_restore`
                значил повторный найм. С 01.09 повторный найм заводит чистую
                карточку сам, и остался ЕДИНСТВЕННЫЙ случай — адрес занят чужой
                активной карточкой. Восстанавливать нечего, и обещать это
                человеку значит отправить его ждать несуществующего действия. */}
            <p className="text-[15px] font-semibold text-text">
              {archived
                ? 'Ваша карточка сотрудника в архиве'
                : 'Карточка сотрудника не найдена'}
            </p>
            <p className="mt-0.5 text-[13px] text-text2">
              {archived
                ? 'Материалы обучения недоступны. Обратитесь к администратору или в отдел персонала.'
                : 'Ваш адрес закреплён за другой карточкой — её нужно разобрать вручную. Обратитесь к администратору или в отдел персонала.'}
            </p>
          </div>
        </div>
      )}
