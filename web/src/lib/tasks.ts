...
export interface Task {
  ...
  /** «Задача лежит в чьём-то личном пространстве» — заполняют кросс-проектные
   *  ручки. Чьём именно, сервер не говорит; клиенту нужен факт, чтобы
   *  подписать строку («Личное» / «Личное коллеги») вместо имени проекта,
   *  которого он не знает. Вывести это из «проекта нет в `useProjects()`»
   *  нельзя: так же выглядит обычный проект, из которого человека убрали. */
  project_is_personal?: boolean | null
  created_at: string
  updated_at: string
  ...
  /** Задача шаблона проекта (0060): просрочки и галочки нет. */
  is_template?: boolean
