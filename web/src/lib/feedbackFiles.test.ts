...
describe('addFeedbackFiles', () => {
  ...
  it('следит за суммарным весом набора, а не только за каждым файлом', () => {
    // По отдельности каждый проходит (18 МБ < 20), втроём — 54 МБ при потолке
    // в 50: третий не берём и говорим, почему именно он.
    const mb18 = 18 * 1024 * 1024
    const { files, errors } = addFeedbackFiles(
      [],
      [mkFile('a.png', mb18, 1), mkFile('b.png', mb18, 2), mkFile('c.png', mb18, 3)],
    )
