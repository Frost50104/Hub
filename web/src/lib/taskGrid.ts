 ...
 * быть не может (поля принадлежат проекту). Средних треков нет вовсе: имя
 * проекта — подпись в строке контекста под заголовком, рядом с чипом колонки
 * (`TaskContextLine.project`), как на телефоне. До 16.09 здесь стоял трек
 * «Проект» 190px: владелец, увидев мобильную пару «проект · колонка», попросил
 * то же на десктопе — в столбце имя стояло в 400px правее заголовка и
