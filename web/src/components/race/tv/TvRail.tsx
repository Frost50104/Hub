...
function Podium({ row, first }: { row: LeaderRow; first: boolean }) {
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 rounded-2xl border border-hair bg-tint px-5 py-4', first && 'race-tv-gold')}>
      <div className="flex min-w-0 items-center gap-3">
        <span className={cn('flex shrink-0 items-center justify-center rounded-full font-display font-bold tabular-nums', MEDAL[place] ?? 'bg-surface text-text2', first ? 'h-12 w-12 text-[24px]' : 'h-10 w-10 text-[20px]')}>
          {place}
        </span>
        ...
        <span className={cn('min-w-0 flex-1 truncate font-semibold text-text', first ? 'text-[28px]' : 'text-[24px]')}>{row.p.name}</span>
      </div>
      <div className="flex items-end justify-between gap-4">
        ...
        <span className={cn('shrink-0 font-display font-bold leading-none tabular-nums text-text', first ? 'text-[44px]' : 'text-[34px]')}>
          {formatPct(row.p.pct)}
        </span>
      </div>
    </div>
...
/** Правая колонка ТВ: подиум топ-3, места 4–10, «…и ещё N». */
export function TvRail({ participants, view, maxRows = 10 }: { participants: RaceParticipant[]; view: RaceView; maxRows?: number }) {
    ...
    return (
      <aside className="flex min-h-0 flex-col gap-3">
        ...
        <div className="flex min-h-0 flex-col gap-2 overflow-hidden">
          {unranked.slice(0, maxRows).map((p) => (
            <div key={p.store_id} className="flex min-h-[56px] items-center gap-3 rounded-xl border border-hair bg-tint px-4">
        ...
        {unranked.length > maxRows && (
          <p className="text-text2" style={{ fontSize: 'var(--race-fs)' }}>
            {nbsp(`…и ещё ${plural(unranked.length - maxRows, 'точка', 'точки', 'точек')}`)}
          </p>
        )}
      </aside>
    )
  ...
  const rest = ranked.slice(3, maxRows)
  const more = ranked.length - Math.min(ranked.length, maxRows)
  return (
    <aside className="flex min-h-0 flex-col gap-3">
      <p className="uppercase tracking-[0.12em] text-text2" style={{ fontSize: 'var(--race-fs)' }}>
        Лидеры заезда
      ...
      <div className="flex min-h-0 flex-col gap-2 overflow-hidden">
        {rest.map((r) => (
          <div key={r.p.store_id} className="flex min-h-[56px] items-center gap-3 rounded-xl border border-hair bg-tint px-4">
      ...
      {more > 0 && (
        <p className="text-text2" style={{ fontSize: 'var(--race-fs)' }}>
          {nbsp(`…и ещё ${plural(more, 'точка', 'точки', 'точек')} — на дорожке`)}
        </p>
      )}
    </aside>
  )
