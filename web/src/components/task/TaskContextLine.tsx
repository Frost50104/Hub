...
interface TaskContextLineProps {
   ...
   * соседствует с чипами — в обеих раскладках (на десктопе с 16.09: отдельный
   * столбец «Проект» владелец попросил заменить этой же парой). Внутри проекта
   * его не передают — там оно дубль.
   */
  project?: string | null
...
export function TaskContextLine({
  ...
  return (
    ...
    >
      ...
      {mode !== 'plain' && stage && (
        // Потолок ширины и `truncate` — везде, где чип соседствует с именем
        // проекта (и в compact вообще): имена колонок бывают длинными — на
        // проде есть живые задачи с колонкой в 50 символов. Без потолка чип
        // (он `shrink-0`) съедал бы строку и оставлял от проекта одну букву.
        <span
          className={cn(
            'inline-flex h-[22px] shrink-0 items-center rounded-md bg-surface px-1.5 text-[12px] font-semibold text-text2',
            (compact || !!project) && 'max-w-[45%] truncate',
          )}
          title={`Колонка: ${stage}`}
        >
          {stage}
